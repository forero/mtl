...
def update_version_py():
    ...
    try:
        ...
        print "unable to run git, leaving py/mtl/_version.py alone"
        return
    out = p.communicate()[0]
    ver = out.rstrip()
    if p.returncode != 0:
        print "unable to run git, leaving py/mtl/_version.py alone"
        return
    f = open("py/mtl/_version.py", "w")
    f.write( '__version__ = \'{}\''.format( ver ) )
    f.close()
    print "Set py/mtl/_version.py to {}".format( ver )


def get_version():
    if not os.path.isfile("py/mtl/_version.py"):
        print 'Creating initial version file'
        update_version_py()
    ver = 'unknown'
    f = open("py/mtl/_version.py", "r")
...
setup (
    ...
    description='DESI Merged Target List',
    author='DESI Collaboration',
    author_email='desi-data@desi.lbl.gov',
    url='https://github.com/forero/mtl',
    package_dir={'':'py'},
